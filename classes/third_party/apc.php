...
function menu_entry($ob,$title) {
	global $MYREQUEST;
	if ($MYREQUEST['OB']!=$ob) {
		return "<li><a href=\"{$GLOBALS['MY_SELF']}&OB=$ob\">$title</a></li>";
	} else if (empty($MYREQUEST['SH'])) {
		return "<li><span class=active>$title</span></li>";
	} else {
		return "<li><a class=\"child_active\" href=\"{$GLOBALS['MY_SELF']}&OB=$ob\">$title</a></li>";
	}
}

function put_login_link($s="Login")
{
	global $MYREQUEST,$AUTHENTICATED;
...
EOB;
		...
		print <<<EOB
			<a href="{$GLOBALS['MY_SELF']}&LO=1&OB={$MYREQUEST['OB']}">$s</a>
EOB;
	}
}
...
echo <<<EOB
	<ol class=menu>
	<li><a href="{$GLOBALS['MY_SELF']}&OB={$MYREQUEST['OB']}&SH={$MYREQUEST['SH']}">Refresh Data</a></li>
EOB;
echo
...
if ($AUTHENTICATED) {
	echo <<<EOB
		<li><a class="aright" href="{$GLOBALS['MY_SELF']}&CC=1&OB={$MYREQUEST['OB']}" onClick="javascript:return confirm('Are you sure?');">Clear $cache_mode Cache</a></li>
EOB;
}
echo <<<EOB
	</ol>
  ...
  echo
	...
	if ($list) {
      ...
      if(!$MYREQUEST['SEARCH'] || preg_match($MYREQUEST['SEARCH'], $entry[$fieldname]) != 0) {
        ...
        echo
          '<tr class=tr-',$i%2,'>',
          "<td class=td-0><a href=\"{$GLOBALS['MY_SELF']}&OB=",$MYREQUEST['OB'],"&SH=",md5($entry[$fieldkey]),"\">",$field_value,'</a></td>',
        ...
        if ($entry['deletion_time']) {
          ...
          echo '[<a href="', $GLOBALS['MY_SELF'], '&OB=', $MYREQUEST['OB'], '&DU=', urlencode($entry[$fieldkey]), '">Delete Now</a>]';
          echo '</td>';
        } else {
...
EOB;
	...
	if ($list && $i < count($list)) {
		echo "<a href=\"{$GLOBALS['MY_SELF']}&OB=",$MYREQUEST['OB'],"&COUNT=0\"><i>",count($list)-$i,' more available...</i></a>';
	}
